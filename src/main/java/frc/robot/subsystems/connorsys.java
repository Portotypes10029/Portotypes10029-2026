...
public class connorsys extends SubsystemBase {
     ...
     */
    public final Timer poopoo = new Timer();
    public boolean poopshooter = false;
    public void drewlacoss(){
        if (!poopshooter){
            poopoo.reset();
            poopoo.start();
            poopshooter = true;
        }
        double butt = poopoo.get();
        if(butt<2){
            leftShooter.setControl(shooterSpeed);
            rightShooter.setControl(shooterSpeedReversed);
